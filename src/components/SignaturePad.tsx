    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={handleClear} hitSlop={8} disabled={isEmpty}>
          <Text style={[styles.clearText, isEmpty && styles.clearTextDisabled]}>Clear</Text>
        </TouchableOpacity>
      </View>

      <View style={styles.canvasWrap}>
        <View ref={canvasRef} collapsable={false} style={styles.canvas} {...panResponder.panHandlers}>
          <Svg style={StyleSheet.absoluteFill}>
            {paths.map((d, i) => (
              <Path key={i} d={d} stroke={colors.text} strokeWidth={3} fill="none" strokeLinecap="round" strokeLinejoin="round" />
            ))}
            {currentPath ? (
              <Path d={currentPath} stroke={colors.text} strokeWidth={3} fill="none" strokeLinecap="round" strokeLinejoin="round" />
            ) : null}
          </Svg>
        </View>
        {isEmpty && (
      ...
      <TouchableOpacity
        ...
        <Text style={styles.doneButtonText}>{saving ? 'Saving…' : 'Use This Signature'}</Text>
      </TouchableOpacity>
    </View>
  );
}
