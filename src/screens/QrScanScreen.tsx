...
export default function QrScanScreen({ navigation }: Props) {
  ...
  async function captureAndScan() {
    try {
      ...
      setLoading(true);
      const barcodes = await scanBarcodesFromImage(scannedImages[0]);
      setResults(barcodes);
    } catch (error) {
      Alert.alert('Scan failed', 'Could not read a QR code or barcode from that photo.');
      setResults([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <TouchableOpacity onPress={captureAndScan} disabled={loading}>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          ...
          <TouchableOpacity style={styles.retryButton} onPress={captureAndScan}>
