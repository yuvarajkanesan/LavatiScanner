...
export default function QuickTextScreen({ navigation }: Props) {
  ...
  async function captureAndRecognize() {
    try {
      ...
      setLoading(true);
      const recognized = await recognizeTextFromImage(scannedImages[0]);
      setText(recognized);
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <TouchableOpacity onPress={captureAndRecognize} disabled={loading}>
          <Icon name="replay" size={22} color={colors.accent} />
        </TouchableOpacity>
      </View>

      {imageUri && (
        <Image source={{ uri: `file://${imageUri.replace('file://', '')}` }} style={styles.thumb} resizeMode="cover" />
      )}
